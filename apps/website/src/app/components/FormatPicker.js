...
export default function FormatPicker({ videoInfo, onSelect, onBack, downloadFolder, onChooseFolder }) {
  ...
  return (
    <div className="space-y-5">
      ...
      {/* Video info card */}
      <div className="flex flex-col sm:flex-row gap-4 p-4 rounded-gb bg-gb-surface border border-gb-border">
        {thumbnail && (
          <div className="shrink-0 w-full sm:w-40 h-32 sm:h-[90px] rounded-gb-sm overflow-hidden bg-gb-surface-light relative">
            <img src={thumbnail} alt={title} className="w-full h-full object-cover"
              onError={(e) => { e.target.style.display = 'none'; }} />
          </div>
        )}
        <div className="flex-1 min-w-0 flex flex-col justify-center">
          <h2 className="text-sm font-semibold text-gb-text sm:truncate leading-snug break-words" title={title}>{title}</h2>
          {channel && <p className="text-xs text-gb-text-muted mt-1 truncate">{channel}</p>}
        ...
        <div className="space-y-2">
          {formats.map((fmt) => (
            <button key={fmt.id} onClick={() => onSelect(fmt)}
              ...
              className={`w-full flex items-center gap-3 px-4 py-3 rounded-gb border transition-all duration-200 cursor-pointer
                ${hoveredFormat === fmt.id
                  ? 'bg-gb-surface-light border-gb-accent/30 shadow-[0_4px_24px_rgba(0,0,0,0.4)]'
      ...
      <div className="flex items-center justify-between px-3 py-2.5 rounded-gb-sm bg-gb-surface border border-gb-border text-xs">
        <div className="flex items-center gap-2 text-gb-text-muted min-w-0">
          ...
          <span className="truncate">{downloadFolder || 'Downloads folder'}</span>
        </div>
        <button onClick={onChooseFolder}
          className="text-gb-accent hover:text-gb-accent-hover transition-colors font-medium shrink-0 ml-2 cursor-pointer"
